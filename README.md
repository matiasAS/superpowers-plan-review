...
- **`plan`** — write a new plan (via `superpowers`) or open an existing one, then **review and refine it interactively** on the repo's `.md`. Finishing the plan never starts implementation.
- **`pal`** — run an adversarial **Red Team Consensus** (via the PAL MCP) over a specific plan, apply only code-verified fixes, then open the interactive review.
...
- The [`superpowers`](https://github.com/obra/superpowers) plugin — the `plan` skill calls `superpowers:brainstorming` and `superpowers:writing-plans`.
- The `pal` MCP server — only needed for the `pal` skill; it provides the `consensus`, `chat`, and `listmodels` tools.
...
- **Write or review a plan:** `/superpowers-plan-review:plan`, or just ask — e.g. "let's review the plan", "write an implementation plan for X".
- **Red team a plan:** `/superpowers-plan-review:pal`, or ask to "red team the plan".
